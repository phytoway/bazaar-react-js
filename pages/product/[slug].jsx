...
import api2 from "utils/__api__/review";
...
const ProductDetails = props => {
  ...
  return <ShopLayout1>
      ...
      my: 4
        ...
        {product && review ? <ProductIntro product={product[0]} review={review} /> : <H2>Loading...</H2>}


        {/* PRODUCT DESCRIPTION AND REVIEW */}
        <StyledTabs textColor="primary" value={selectedOption} indicatorColor="primary" onChange={handleOptionClick}>
          <Tab className="inner-tab" label="Description" />
          <Tab className="inner-tab" label={`Review (${review.total_count})`} />
        </StyledTabs>

        <Box mb={6}>
          {selectedOption === 0 && <ProductDescription productsData={product[0]}/>}
          {selectedOption === 1 && <ProductReview review={review}/>}
        </Box>
...
export const getStaticProps = async ({
  ...
  //console.log('상품번호:' + product[0].id);
  const review = await api2.getReview({product_id:product[0].id});
  return {
    props: {
      frequentlyBought,
      relatedProducts,
